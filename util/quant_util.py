...
def quantize_to_float8(tensor):
    # Define the float8 range
    float8_max = torch.tensor(1.0, dtype=torch.float32, device = 'cuda')
    float8_min = torch.tensor(-1.0, dtype=torch.float32, device = 'cuda')
    
    # Clamp the values to be within the range of float8
    clamped_tensor = torch.clamp(tensor, float8_min, float8_max)
    
    # Convert to float8
    quantized_tensor = clamped_tensor * 127.0
...
def quantize_to_float16(tensor):
    # Quantize the float32 tensor to float16
    quantized_tensor = tensor.to(torch.float16)
    return quantized_tensor
